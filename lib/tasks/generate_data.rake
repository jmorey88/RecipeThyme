...
namespace :data do
  namespace :generate do
    CATEGORIES = [
      'Breakfast', 'Lunch', 'Entrees', 'Desserts', 'Beverages',
      'Appetizers', 'Breads', 'Salads', 'Soups', 'Miscellaneous'
    ].freeze
    ...
    task recipe_titles: :environment do
      ...
      existing_data = if File.exist?(file_path) && !File.zero?(file_path)
                        JSON.parse(File.read(file_path))
                      else
                        []
                      end

      # categories = %w[Breakfast Lunch Entrees Desserts Beverages Appetizers Breads Salads
      #                 Soups Miscellaneous]
      prompt = "Generate 200 unique and creative recipe titles suitable for a diverse cooking blog, covering the following categories: #{CATAGORIES.join(', ')}. Each title be distinct, concise, and not exceed 35 characters. Avoid prefixes, numbers, dashes, colons, repeated words or special formatting in the titles.  Example: Sunrise Berry Yogurt Parfait"
      new_recipes = []
      ...
      new_recipes = titles.map { |title| { title: } }
    ...
    task tag_arrays: :environment do
      ...
      recipes.each do |recipe|
        ...
        # valid_categories = %w[Breakfast Lunch Entrees Desserts Beverages Appetizers Breads Salads
        #                       Soups Miscellaneous]
        title = recipe['title']
        prompt = "Here is a recipe title: '#{title}'. Based on its content, list the appropriate categories from the following: #{CATAGOREIS.join(', ')}. Please format your response as a comma-separated list of categories."

        categorized_titles = send_prompt_to_gpt(prompt)

        category_list = categorized_titles.split(',').map(&:strip)

        recipe['tag_names'] = category_list.select do |category|
...
def send_prompt_to_gpt_for_image(title)
  ...
  response_body = JSON.parse(response.body)
  return unless response_body['data'] && !response_body['data'].empty?
